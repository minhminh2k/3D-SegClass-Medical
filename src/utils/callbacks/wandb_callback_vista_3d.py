...
class LIDC_Vista_3D_Callback(Callback):
    def __init__(
        self,
        ...
        coefficient: int = 5,
        label_mappings: dict = {"default": [[1, 23]]},
        sw_batch_size: int = 1,
        label_set: list = [0, 23]
    ):
    ...
    def on_train_epoch_end(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule"):
        ...
        #             trainer.model.net,
        #             transpose=True,
        #             class_vector=torch.tensor(self.label_set[1:], device=trainer.model.device),
        #         )
        #         outputs = VistaPostTransformd(keys="pred")(
        #             {"pred": outputs, "label_prompt": torch.tensor(self.label_set[1:], device=trainer.model.device)}
        #         )["pred"]

                    
        #         seg = outputs[0].detach().cpu().numpy().astype(np.int8) # [1, 128, 128, 128]
        #         label = label.cpu().numpy().astype(np.int8)

        #         for idx, post_label in enumerate(self.label_set):
        #             seg[seg == post_label] = idx
        #             label[label == post_label] = idx
    ...
    def on_validation_epoch_end(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule"):
        ...
        with torch.no_grad():
            for batch in self.n_samples_validation:
                ...
                label = batch["label"].numpy().astype(np.int8) # [128, 128, 128]                
                prob = batch["preds"].cpu().numpy().astype(np.int8)

                for idx, post_label in enumerate(self.label_set):
                    prob[prob == post_label] = idx
                    label[label == post_label] = idx
