...
class TransformerBTS(nn.Module):
    def __init__(
        ...
        self.num_patches = int(
            (img_dim[0] // patch_dim[0])
            * (img_dim[1] // patch_dim[1])
            * (img_dim[2] // patch_dim[2])
        )
...
def TransBTS(img_dim, num_classes=2, _conv_repr=True, _pe_type="learned"):
    num_channels = 1
    patch_dim = [8, 8, 8]
    aux_layers = [1, 2, 3, 4]
    model = BTS(
...
if __name__ == "__main__":
    with torch.no_grad():
        ...
        x = torch.rand((1, 1, 96, 128, 128), device=cuda0)
        model = BTS(
            img_dim=96,
            patch_dim=8,
            num_channels=1,
